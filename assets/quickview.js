...
$(document).ready(function () {
  $.getScript("//cdnjs.cloudflare.com/ajax/libs/fancybox/2.1.5/jquery.fancybox.min.js").done(function(){ quickView(); });
});
 ...
 $(".quick-view").click(function () {
   ...
   jQuery.getJSON('/products/' + product_handle + '.js', function (product) {
     ...
     $(options).each(function (i, option) {
       ...
       $('.qv-product-options').append('<div class="option-selection-' + opt.toLowerCase() + '"><span class="option">' + opt + '</span><select class="option-' + i + ' option ' + opt.split(" ").join("").toLowerCase() + '"></select></div>');
       $(option.values).each(function (i, value) {
         console.log(opt);
          
         console.log('.option.' + opt.toLowerCase());
         $('.option.' + opt.split(" ").join("").toLowerCase()).append('<option value="' + value + '">' + value + '</option>');
       });
     });
     $(product.variants).each(function (i, v) {
       if (v.inventory_quantity == 0) {
         ...
         if (original_price > 0) {
           $('.qv-product-original-price').html("$"+ original_price).show();
         } else {
     ...
     'beforeLoad': function () {
       ...
       $(document).on("click", ".qv-add-button", function () {
         ...
         function processCart() {
           jQuery.post('/cart/add.js', {quantity: qty,id: var_id},null,"json").done(function (res) {
              document.documentElement.dispatchEvent(new CustomEvent('cart:refresh', {
                bubbles: true
              }));
              document.dispatchEvent(new CustomEvent("product:added", { detail: { product: res} }));
           }).fail(function ($xhr) {
...
$(document).on('click','.QuantitySelector__Button',function(){
  var actionRead = $(this).attr('data-action'),currentValue = parseInt($(this).parent().find('.QuantitySelector__CurrentQuantity').val());
  if(actionRead == "decrease-quantity")
  {
    if(currentValue > 0 ){
  ...
  $(this).parent().find('.QuantitySelector__CurrentQuantity').val(currentValue);
}); 